...
function desiredWeek(start){const weekNo=weekNumber(start),active=preferredDays(weekNo),activeDates=[];for(let i=0;i<7;i++){const d=addDays(start,i);if(active.includes(d.getDay()))activeDates.push(d);}const n=activeDates.length,plans=[];if(n===1){plans.push(HIP_A);}else if(n===2){plans.push(HIP_A,...extrasForWeek(weekNo,1));}else if(n===3){plans.push(HIP_A,...extrasForWeek(weekNo,1),HIP_B);}else if(n>=4){const extras=extrasForWeek(weekNo,n-2),secondCore=Math.floor(n/2),e=0;for(let i=0;i<n;i++){if(i===0)plans.push(HIP_A);else if(i===secondCore)plans.push(HIP_B);else plans.push(extras[e++]);}}
